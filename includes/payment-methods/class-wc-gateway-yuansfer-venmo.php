...
/**
 * Class that handles Venmo payment method.
 *
 * @extends WC_Gateway_Yuansfer
 *
 */
class WC_Gateway_Yuansfer_Venmo extends WC_Yuansfer_Payment_Gateway {

	/**
	 * Constructor
	 */
	public function __construct() {
		$this->id                   = 'yuansfer_venmo';
		$this->method_title         = __('Yuansfer Venmo', 'woocommerce-yuansfer');
	...
	public function init_form_fields() {
		$this->form_fields = require(WC_YUANSFER_PLUGIN_PATH . '/includes/admin/yuansfer-venmo-settings.php');
	}
	...
	public function payment_fields() {
		...
		echo '<div
			id="yuansfer-venmo-payment-data"
    ...
    public function create_source($order) {
        ...
        if (!in_array($currency, $supportedCurrency, true)) {
            throw new WC_Yuansfer_Exception('Venmo only support "' . implode('", "', $supportedCurrency). '" for currency');
        }

        $post_data['amount']      = WC_Yuansfer_Helper::get_yuansfer_amount($order->get_total(), $currency);
        $post_data['currency']    = $currency;
        $post_data['settleCurrency'] = 'USD';
        $post_data['vendor']      = 'venmo';
        $post_data['reference']   = $order_id . ':' . uniqid('venmo:');
    ...
    public function process_payment($order_id, $retry = true, $force_save_source = false) {
        try {
            ...
            WC_Yuansfer_Logger::log('Info: Redirecting to Venmo...');
